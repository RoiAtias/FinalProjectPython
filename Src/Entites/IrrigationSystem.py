...
class IrrigationSystem:
    ...
    def irrigate_plants(self, plant_list: list[Plant]):
        try:
            if len(plant_list) > 0:
                amount_water_irrigation = 0

                if self.water_level > 0:
                    amount_water_irrigation = self.water_level / len(plant_list)
    ...
    def check_water_level_in_irrigation_system(self, plant_list: list[Plant]):
        is_valid = True
        sum_water_requirement = sum(plant.water_requirement for plant in plant_list)
